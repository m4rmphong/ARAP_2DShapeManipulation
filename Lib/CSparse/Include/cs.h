...
typedef struct cs_symbolic  /* symbolic Cholesky, LU, or QR analysis */
{
    ...
    float lnz ;    /* # entries in L for LU or Cholesky; in V for QR */
    float unz ;    /* # entries in U for LU; in R for QR */
} css ;
...
csi cs_pvec (const csi *p, const float *b, float *x, csi n) ;
...
csi cs_usolve (const cs *U, float *x) ;
...
float cs_cumsum (csi *p, csi *c, csi n) ;
...
csi cs_scatter (const cs *A, csi j, float beta, csi *w, float *x, csi mark,
    cs *C, csi nz) ;
csd *cs_scc (cs *A) ;
csi cs_spsolve (cs *G, const cs *B, csi k, csi *xi, float *x,
